...
public class Gclub {
    ...
    public Object userStories(@RequestParam(defaultValue = "5") int pageSize) {
        ...
        searchCriteria.put("field", "title");
        searchCriteria.put("condition_type", "eq");
        searchCriteria.put("value", "gclub-stories");
        searchCriteria.put("pageSize", String.valueOf(pageSize));

        String body = this.magentoClientAdvanced.getForBody("rest/default/V1/cmsPage/search" + QUERY_STRING, searchCriteria);

        //通过 反序列化，去掉不需要返回的字段
        //TODO:文章列表应该有 缩略图 字段
        CmsPageList cmsPageList = (new Gson()).fromJson(body, CmsPageList.class);
    ...
    public Object events(@RequestParam(defaultValue = "5") int pageSize) {
        ...
        searchCriteria.put("field", "title");
        searchCriteria.put("condition_type", "eq");
        searchCriteria.put("value", "gclub-events");
        searchCriteria.put("pageSize", String.valueOf(pageSize));
    ...
    public Object favourites(@RequestParam(defaultValue = "5") int pageSize) {
        ...
        searchCriteria.put("field", "title");
        searchCriteria.put("condition_type", "eq");
        searchCriteria.put("value", "gclub-favourites");
        searchCriteria.put("pageSize", String.valueOf(pageSize));
    ...
    public Object userLevels() {
        Map<String, String> searchCriteria = new HashMap<>();
        searchCriteria.put("field", "customer_group_code");
        searchCriteria.put("condition_type", "like");
        searchCriteria.put("value", "gclub-level%");

        String body = this.magentoClientAdvanced.getForBody("rest/default/V1/customerGroups/search" + QUERY_STRING, searchCriteria);

        //通过 反序列化，去掉不需要返回的字段
        UserGroupList userGroupList = (new Gson()).fromJson(body, UserGroupList.class);
    ...
    public CustomerEntity me(@RequestHeader("Authorization") String token) {
        ...
        return (new Gson()).fromJson(body, CustomerEntity.class);
    }
    ...
    public CmsPageContent getPage(int pageId) {
        ...
        //通过 反序列化，去掉不需要返回的字段
        return (new Gson()).fromJson(body, CmsPageContent.class);
    }
}
