...
import com.gfashion.data.repository.elasticsearch.ConfigurationInfo;
...
import com.gfashion.data.repository.elasticsearch.handlers.ElasticSearchClientHandler;
...
public class ElasticSearchService {
    ...
    private void signRequest(Request request) {
        ...
        aws4Signer.sign(request, configurationInfo.awsCredentialsProvider().getCredentials());
    }
    ...
    public String createNewProduct(Product product) throws JsonProcessingException {
        ...
        if (json != null) {
            ...
            if (response != null && (response.getHttpResponse().getStatusCode() == HttpStatus.CREATED.value()
                ...
                LOGGER.info("Successfully created new product with ID: {} and title: {}", product.getId(), product.getName());
    ...
    private void createProductQuery(final ProductQuery productQuery, JSONArray array) {
        if(productQuery.getId() != null){
            buildElasticSearchMatchStatement("id", productQuery.getId(), array);
        }
        if(checkNullOrEmpty(productQuery.getName())){
            buildElasticSearchMatchStatement("title", productQuery.getName(), array);
        }
        if(productQuery.getPrice() > 0){
            buildElasticSearchMatchStatement("price", productQuery.getPrice(), array);
        }
        if(checkNullOrEmpty(productQuery.getBrandName())){
            buildElasticSearchMatchStatement("brandName", productQuery.getBrandName(), array);
        }
        if(checkNullOrEmpty(productQuery.getProductBrief())){
            buildElasticSearchMatchStatement("productBrief", productQuery.getProductBrief(), array);
        }
    }
    ...
    private void createProductQueryFuzzySearch(final ProductQuery productQuery, JSONObject searchTerm) {
        if(checkNullOrEmpty(productQuery.getProductBrief())){
            buildElasticSearchFuzzyStatement("productBrief", productQuery.getProductBrief(), searchTerm);
        }
    }
